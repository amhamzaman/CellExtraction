...
class Stacks():
    def __init__(self, input_path, output_path):
        ...
        # self.TARGET_FUNC = 'self.load_images'
        self.IMAGES = self.load_images(self.FILENAMES)

    def load_images(self, filenames):
        # images_list = []
        # image = cv2.imread(filename,cv2.IMREAD_GRAYSCALE)
        images_list = [cv2.imread(i, cv2.IMREAD_GRAYSCALE) for i in filenames]
        return images_list

    def read_images_from_path(self):
        file_names = natsorted([img for img in glob.glob(
...
class RegionalFusion():
    def __init__(self, images, input_path, output_path, pyramid_depth, kernel_size):
        ...
        fused_img = cv2.pyrUp(base, dstsize=(
            LP_f[-1].shape[1], LP_f[-1].shape[0])).astype(np.int16)

        for i in reversed(range(self.PYRAMID_DEPTH)):
            # combine with laplacian pyramid at the level
            fused_img += LP_f[i]
            if i != 0:
                fused_img = cv2.pyrUp(fused_img, dstsize=(
                    LP_f[i-1].shape[1], LP_f[i-1].shape[0]))

        self.CANVAS = fused_img

    def extract_pyramids_and_base(self, images):
        temp_list = LaplacianPyramid(
            images, self.INPUT_PATH, self.OUTPUT_PATH, self.PYRAMID_DEPTH).LAP_PYR_LIST_BASE
    ...
    def region_energy_map(self, l, list_lap_pyramids):
        ...
        RE_l = np.array([self.region_energy(lap)
                        for lap in list_lap_pyramids[:, l]], dtype='float32')
    ...
    def compute_LP_l(self, l, list_lap_pyramids, RE_max_idx):
        ...
        stacks_at_l = np.reshape(np.concatenate(stacks_at_l), (len(
            stacks_at_l), stacks_at_l[0].shape[0], stacks_at_l[0].shape[1]))
        LP_l = stacks_at_l[RE_max_idx, np.arange(stacks_at_l[0].shape[0])[
            :, None], np.arange(stacks_at_l[0].shape[1])]
    ...
    def entropy(self, image, kernel_size):
        def get_probabilities(gray_image):
            levels, counts = np.unique(
                gray_image.astype(np.uint8), return_counts=True)
...
def Stacker(func):
    def wrapper(*args, **kwargs):  # Function Wrapper
        print(f'Stacking {args[2]} frames..')
        canvas = func(*args, **kwargs)
        print(f'{args[2]} stacking complete.')
        return canvas
    return wrapper


@Stacker  # Decorator
def init_stacking(input_path, output_path, signal, section_size=1000):
    images = Stacks(input_path, output_path).IMAGES
    h_sections = np.ceil(images[0].shape[0]/section_size).astype('uint8')
    w_sections = np.ceil(images[0].shape[1]/section_size).astype('uint8')

    canvas = np.zeros((images[0].shape[0], images[0].shape[1]), dtype='int16')

    i = 0
    for h in range(h_sections):
        for w in range(w_sections):
            i += 1
            roi_stack = [images[l][h*section_size:(h+1)*section_size, w*section_size:(
                w+1)*section_size] for l in range(0, len(images))]
            canvas[h*section_size:(h+1)*section_size, w*section_size:(w+1)*section_size] = RegionalFusion(
                roi_stack, input_path, output_path, pyramid_depth, kernel_size).CANVAS
